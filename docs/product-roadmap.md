...
| Map Explore Mode — browse and click to plan | V2. An interactive map view as an alternative to the list-based Explore Mode. User clicks a region or destination on the map → triggers the explore/generate flow for that location. Could use Mapbox or Google Maps JS API. Two interaction models to evaluate: (1) click a pinned suggestion on the map after the LLM returns results; (2) click anywhere on the map to seed the destination directly. Pairs with region hint (TRIP-V1-01) — the map click becomes the region filter. Significant frontend complexity; deserves its own epic in V2. |
| TRIP-018: Map Mode — click a destination to plan | V2. Third home screen mode alongside Plan a trip and Explore destinations. User clicks anywhere on an interactive Mapbox map → pin drops + reverse geocode resolves to nearest city (e.g. "Barcelona, Spain") → "Plan this destination" pre-fills PlanForm and navigates there. Requires Mapbox public token (`NEXT_PUBLIC_MAPBOX_TOKEN`), `react-map-gl`, and Mapbox Geocoding API. ACs: pin on click, tooltip with city name + confirm button, move pin before confirming, graceful error if no city resolved. Out of scope: region-hint explore flow, saved pins. |
